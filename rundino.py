...
dino = [pygame.image.load(os.path.join('image', 'rex1.png')), pygame.image.load(os.path.join(
	'image', 'rex2.png')), pygame.image.load(os.path.join('image', 'rex3.png')), pygame.image.load(os.path.join('image', 'rex4.png'))]
dinofire = [pygame.image.load(os.path.join('image', 'trex1.png')), pygame.image.load(os.path.join(
	'image', 'trex2.png')), pygame.image.load(os.path.join('image', 'trex3.png')), pygame.image.load(os.path.join('image', 'trex4.png'))]
dino_lie = [pygame.image.load(os.path.join(
	'image', 'rexlie1.png')), pygame.image.load(os.path.join('image', 'rexlie2.png'))]
...
button1 = pygame.transform.scale(pygame.image.load(
	os.path.join('image', 'start1.png')), (40, 40))
button2 = pygame.transform.scale(pygame.image.load(
	os.path.join('image', 'start2.png')), (40, 40))
dino_start = pygame.transform.scale(
	pygame.image.load(os.path.join('image', 'rex5.png')), (50, 50))
...
def draw_win(high_score, x, y, islie):
	...
	win.blit(highscore, (300-highscore.get_width()//2, 150 -
	         highscore.get_height()//2+gameover.get_height()//2+20))
	if not islie:
...
def main():
	...
	while run:
		...
		BIRD = win.blit(pygame.transform.scale(
			bird, (40, 25)), (X+600+x_bird, y_bird))
		if not weapon:
		...
		for i in range(x_co):
			if i % 2 != 0:
				...
				Tree = win.blit(pygame.transform.scale(
					tree, (20, 40 + i*10*plus)), (X+600 + (i+1)*25, 230 - i*10*plus))
			else:
				Tree = win.blit(pygame.transform.scale(
					tree, (20, 40)), (X+600 + (i+1)*25, 230))

		if not islie:
			if not weapon or fire == False:
				if walk_count + 1 >= 4:
					walk_count = 0
				DINO = win.blit(pygame.transform.scale(
					dino[walk_count], (50, 50)), (x, y+5))
			...
			if weapon and fire == True:
				...
				if count + 1 >= 4:
					count = 0
				DINOFIRE = win.blit(pygame.transform.scale(
					dinofire[count], (50, 50)), (x, y))
				sword_sound.play()
				count += 1
			...
			if COUNT + 1 == 2:
				COUNT = 0
			DINOLIE = win.blit(pygame.transform.scale(
				dino_lie[COUNT], (50, 35)), (x, y+20))
			COUNT += 1
...
def pre_main():
	...
	while running:
		...
		BUT = win.blit(button1, (300 - button1.get_width() //
		               2, 150-button1.get_height()//2))
		x += speedx
		y += speedy
		radius += 5
		...
		if BUT.collidepoint(pygame.mouse.get_pos()):
			BUT = win.blit(button2, (300 - button2.get_width() //
			               2, 150-button2.get_height()//2))
